...
class Player {
    ...
    initHandlers() {
        ...
        this.viewReady().then(() => {
            ...
            $(this.view).on('framewise-annotation', () => {
                var ch = this.check();
                if(ch == 'empty_label_type'){
                    //alert('Empty label. Please fill it.');
                    $('#error_message').text('Empty label. Please fill it then press enter to unfocus then t.');
                    return;
                }
                else if(ch == 'label_exists'){
                    $('#error_message').text('Cannot.Label already present in this frame');
                    return;
                }
                $('#error_message').text('');
                var rect = this.view.addRect();
                var {videoWidth, videoHeight, viewWidth, viewHeight} = this.view.video;
                var bounds = {
                    xMin: 0,
                    yMin: 0,
                    xMax: videoWidth,
    ...
    drawOnscreenAnnotations() {
        var txt = '';
        var nb = this.annotationRectBindings.length;
        var time = this.view.video.currentTime;
        for (let {annotation, rect} of this.annotationRectBindings) {
            this.drawAnnotationOnRect(annotation, rect);
            var prev_time = annotation.keyframes[0].time;
            for(var kf = 1;kf < annotation.keyframes.length;kf++){
                if(time >= prev_time && time <= annotation.keyframes[kf].time){
                    txt += annotation.type + ',';
                    break
                }
                prev_time = annotation.keyframes[kf].time;
            }
    ...
    check(){
        ...
        var type = $('#labels').val();
        if(type == ''){
            return 'empty_label_type';
        }
        for(var i in this.annotations){
            var annot = this.annotations[i];
            for(var j in annot.keyframes){
                var kf = annot.keyframes[j];
                if(kf.time == this.view.video.currentTime && annot.type == type){
                    return 'label_exists';
                }
            }
        }
